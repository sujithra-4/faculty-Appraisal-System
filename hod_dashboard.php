...
// ✅ Get HOD department
$stmt = $conn->prepare("SELECT department FROM faculty_roles WHERE faculty_id = ? AND admin_role='HOD'");
$stmt->bind_param("i", $hod_id);
$stmt->execute();
$stmt->bind_result($hod_dept);
$stmt->fetch();
$stmt->close();

// ✅ Handle HOD approval
if (isset($_POST['approve_appraisal_id']) && isset($_POST['hod_or_principal_feedback'])) {
    $approve_id = $_POST['approve_appraisal_id'];
    $feedback = intval($_POST['hod_or_principal_feedback']);
    if ($feedback >= 0 && $feedback <= 5) {
        $update_stmt = $conn->prepare("UPDATE faculty_appraisal SET status='Approved', hod_or_principal_feedback=? WHERE appraisal_id=?");
        $update_stmt->bind_param("ii", $feedback, $approve_id);
        $update_stmt->execute();
        $update_stmt->close();
    }
...
// ✅ Fetch verified appraisals for department
$appraisals = [];
$query = "
SELECT fa.appraisal_id, f.faculty_name, fr.role, fr.designation, fa.appraisal_year, 
...
WHERE fa.status='Verified' 
  AND fa.department=? 
  AND fa.faculty_id != ?   -- ✅ Prevent HOD from seeing own self-appraisal
ORDER BY fa.submitted_on ASC
";
$stmt = $conn->prepare($query);
$stmt->bind_param("si", $hod_dept, $hod_id);   // ✅ Add HOD ID to bind
$stmt->execute();
...
<nav class="navbar navbar-custom fixed-top">
  ...
  <div class="d-flex align-items-center user-info">
      <span class="me-3"><?php echo htmlspecialchars($_SESSION['user_name']); ?> (<?php echo htmlspecialchars($hod_dept); ?>)</span>
...
<div class="sidebar">
    ...
    <button onclick="showSection('approvals')">Appraisal Approval</button>
</div>

<div class="main-content">
    <div id="dashboard">
        <h2 class="text-center">Welcome, <?php echo htmlspecialchars($_SESSION['user_name']); ?>!</h2>
        <p class="text-center fs-5">Department: <?php echo htmlspecialchars($hod_dept); ?></p>
    </div>

    <div id="approvals" style="display:none;">
        <h2 class="text-center mb-4">Appraisals to Approve</h2>
        <?php if (count($appraisals) === 0): ?>
            <div class="alert alert-info text-center">No appraisals pending approval.</div>
        <?php else: ?>
            <div class="row g-4">
                    ...
                    <div class="modal-dialog modal-xl modal-dialog-scrollable">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <?php foreach($questionsA as $i => $q): ?>
                                    <li><?php echo htmlspecialchars($q); ?><br>
                                        ...
                                        elseif ($i==15) {
                                            ?>
                                            <select name="hod_or_principal_feedback" class="form-select d-inline w-auto" required form="approveForm<?php echo $a['appraisal_id']; ?>">
                            ...
                            <div class="modal-footer">
                                <form method="POST" id="approveForm<?php echo $a['appraisal_id']; ?>">
